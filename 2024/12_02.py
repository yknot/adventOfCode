...
def calculate_area_sides(grid):
    ...
    for i, row in enumerate(grid):
        for j, cell in enumerate(row):
            ...
            regions.append((cell, len(region), sides))

    total = 0
    for _, area, sides in regions:
        print(f"Region: {area} * {sides} = {area * sides}")
        total += area * sides
...
def calculate_price(data):
    ...
    total = calculate_area_sides(grid)
    return total
...
res = calculate_price(example_2_1)
assert res == 368, res
